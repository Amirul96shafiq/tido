    ...
    @if ($results->getCategories()->isEmpty())
        @if (\App\Filament\GlobalSearch\GlobalSearchType::tryFromValue($this->type)->hasTypeFilters() && $this->getActiveFiltersCount() > 0)
            <x-empty-state-panel
                ...
                description="No results match the current search or filters. Adjust the criteria, or clear filters to see everything again."
                icon="heroicon-o-magnifying-glass"
                icon-color="primary"
            >
                <x-slot name="actions">
                    <x-filament::button color="primary" wire:click="clearFilters" type="button">
                        Clear Filters
                    </x-filament::button>
                </x-slot>
            </x-empty-state-panel>
