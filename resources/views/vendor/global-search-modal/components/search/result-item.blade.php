...
@php
    $classes = [
        ...
        'my-1 flex items-center justify-between rounded-lg px-3 py-2 transition-colors duration-300',
    ];

    $isAssoc = \Illuminate\Support\Arr::isAssoc($result->details);
    $searchQuery = trim((string) ($this->search ?? ''));
    $highlightClasses = 'text-primary-500 font-semibold hover:underline';
@endphp
                    ...
                    >
                        ...
                        <dd class="inline">
                            @php
                                ...
                                $highlightedValue = \CharrafiMed\GlobalSearchModal\Utils\Highlighter::make(
                                    text: $escapedValue,
                                    pattern: $searchQuery,
                            ...
                            {{ str($highlightedValue)->sanitizeHtml()->toHtmlString() }}
                        </dd>
                    </div>
                @endforeach
            </dl>
