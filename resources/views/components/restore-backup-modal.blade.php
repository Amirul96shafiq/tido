...
@if (\App\Models\User::query()->doesntExist())
    ...
    >
        <div
            class="absolute inset-0 bg-gray-950/50 backdrop-blur-md transition-opacity dark:bg-gray-950/75"
            x-on:click="close()"
            aria-hidden="true"
        ></div>
        ...
        <div
            ...
            class="pointer-events-auto relative mx-auto flex w-full max-w-lg cursor-default flex-col overflow-hidden rounded-xl bg-white shadow-xl ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10"
        >
            <div class="flex items-center justify-between border-b border-gray-200 px-6 pt-6 pb-4 dark:border-gray-700">
            ...
            <form class="space-y-5 px-6 py-5" @submit.prevent="submit()">
                <div class="space-y-2">
                    ...
                    <input
                        ...
                        class="file:bg-primary-600 hover:file:bg-primary-500 block w-full text-sm text-gray-700 file:me-4 file:rounded-lg file:border-0 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-white dark:text-gray-200"
                        x-on:change="onFileChange($event)"
                    />
                    <p class="text-xs text-gray-500 dark:text-gray-400">
                        .zip only · max {{ (int) ceil(((int) config('backup.backup.restore.max_upload_kilobytes', 51200)) / 1024) }} MB
                ...
                <div class="flex items-center justify-end gap-3 pt-2">
                    <button
                        type="button"
                        class="fi-btn fi-color-gray relative inline-grid grid-flow-col items-center justify-center gap-1.5 rounded-lg bg-white px-3 py-2 text-sm font-semibold text-gray-700 shadow-sm ring-1 ring-gray-950/10 transition duration-75 outline-none hover:bg-gray-50 dark:bg-white/5 dark:text-gray-200 dark:ring-white/20 dark:hover:bg-white/10"
                        x-on:click="close()"
                        :disabled="submitting"
                    >
                        Cancel
                    ...
                    <button
                        type="submit"
                        class="fi-btn fi-color-danger bg-danger-600 hover:bg-danger-500 focus-visible:ring-danger-600/50 relative inline-grid grid-flow-col items-center justify-center gap-1.5 rounded-lg px-3 py-2 text-sm font-semibold text-white transition duration-75 outline-none focus-visible:ring-2 disabled:opacity-70"
                        :disabled="submitting"
                    >
            ...
            >
                <div class="flex gap-3 p-4">
                    ...
                    <div class="fi-no-notification-text grid flex-1 gap-y-1">
                        ...
                        ></h3>
                        <div
                            class="fi-no-notification-body overflow-hidden text-sm break-words text-gray-500 dark:text-gray-400"
