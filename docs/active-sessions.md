...
The section must keep `->id('active-sessions')` so hash navigation, the sticky section tabs, and `<x-hash-scroll />` work in SPA mode. See [`ui-section-nav.md`](ui-section-nav.md).
