...
* **Application / DB / queues:** Windows host development runs PHP via `npm run dev:full` with SQLite (default) and a `database` queue connection. `queue:listen` handles `default`, `whatsapp`, and `receipts`. `reverb:start` serves websockets on **8081** for Expenses table live sync (see [`realtime-broadcasting.md`](realtime-broadcasting.md)).
...
* **Reverb (Echo):** Native host process on `http://127.0.0.1:8081`. Filament Echo refreshes List Expenses on expense create/status change. Do not use Pusher Cloud.
