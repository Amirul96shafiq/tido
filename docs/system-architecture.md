...
On databases where **MH-004** is Verified, all pre-migration rows live in household #1. New households are created via `HouseholdRegistrationService` (**MH-007**); public Sign Up is on the `/admin/login` Sign Up tab (email/password + email OTP). Google sign up there is coming soon. Active tenancy work: see the top **Open or In progress** row in [multi-household-change-checklist.md](multi-household-change-checklist.md).
