...
* **Reverb (Echo):** Native host process on `http://127.0.0.1:8081`. Filament Echo refreshes List Expenses, Upload Receipts, and Recent Receipts on expense create/status change, and the database-notifications inbox on `DatabaseNotificationsSent`. Do not use Pusher Cloud.
