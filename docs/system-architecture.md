...
* **Currency evidence:** Prompts must preserve a printed ISO code or unambiguous marker, keep source amounts unchanged through extraction, and return null for missing or ambiguous currency. A failed or pending conversion remains source-denominated and is excluded from MYR analytics.
