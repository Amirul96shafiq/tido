...
* **Household access:** Single panel with Primary vs Family Member roles (`HouseholdRole`); family members mutate only their attributed expenses. Resource edit audit records the authenticated Primary or Family Member separately from expense spender attribution. See `docs/household-access.md` and `docs/resource-edit-audit.md`.
...
* **Application / DB / queues:** Windows host development runs PHP via `npm run dev:full` with SQLite (default) and a `database` queue connection. `queue:listen` handles `default`, `whatsapp`, and `receipts`. `reverb:start` serves websockets on **8081** for expense table live sync (see [`realtime-broadcasting.md`](realtime-broadcasting.md)).
...
* **Reverb (Echo):** Native host process on `http://127.0.0.1:8081`. Filament Echo refreshes List Expenses, Upload Receipts, and Recent Receipts on expense create/status change. Do not use Pusher Cloud.
