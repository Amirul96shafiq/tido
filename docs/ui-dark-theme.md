...
| Custom scrollbar thumb (dark)         | slate-700 @ 50% → hover slate-600 @ 70%   | `.fi-dropdown-panel`, other Filament slide-overs, centered action modals, page scroller. Track is the surface (`white` / `slate-800` via `--tido-scrollbar-track`), not transparent and not the OS gray gutter. Nested `.fi-sidebar-nav` / `.custom-scrollbar` / `.fi-ta-content-ctn` / changelog `.fi-modal-window` / notification `.fi-modal-content` use `scrollbar-color` only in Chromium (no `scrollbar-width: thin`, no `::-webkit-scrollbar`) so the OS thumb stays overlay-smooth; track is `white` / `slate-800`. Firefox nested bars are thin inside `@supports not selector(::-webkit-scrollbar)`. Collapsed sidebar hides the nav gutter. |
...
5. **Scrollable panels** — Add `custom-scrollbar` on custom `overflow-y-auto` regions. Filament slide-overs (View details, resource View) are themed via `.fi-modal-slide-over:not(.fi-changelog) … > .fi-modal-window` in `app.css`. Changelog and database notifications use the overlay nested treatment (changelog `.fi-modal-window`, notifications `.fi-modal-content`) so Chromium does not paint a classic 6px gutter. Centered action modals (Ollama Edit setup / Configure) scroll on `.fi-modal-window-ctn` and are themed via `.fi-modal:not(.fi-modal-slide-over) > .fi-modal-window-ctn`. Do not put `custom-scrollbar` on an inner non-scrolling list. Filament `.fi-dropdown-panel` scrollbars are already themed. Nested `.fi-sidebar-nav`, `.custom-scrollbar`, resource-table `.fi-ta-content-ctn`, changelog, and notification panels tint the OS bar with `scrollbar-color` (thumb + surface track) — do **not** add Chromium `::-webkit-scrollbar` or `scrollbar-width: thin` on those, or nested wheel scrolling becomes a classic gutter.
