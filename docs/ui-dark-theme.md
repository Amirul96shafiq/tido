...
4. **Tooltips** — Do not use Tippy’s default `#333` or Chart.js `#333333`. Tippy is overridden in `app.css`; charts read `--color-slate-700` in `filament-chart-js-plugins.js`. Icon CTAs must use Filament Tippy (`->tooltip()` / `x-tooltip`), not browser `title` — see [ui-tooltips.md](ui-tooltips.md). Custom modals at `z-index: 99999` (restore backup) need Tippy `zIndex: 100000` or tooltips render behind the shell.
5. **Scrollable panels** — Add `custom-scrollbar` on custom `overflow-y-auto` regions (e.g. changelog slide-over). Filament `.fi-dropdown-panel` scrollbars are already themed in `app.css`.
