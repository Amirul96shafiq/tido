...
| Custom scrollbar thumb (dark)         | slate-700 @ 50% → hover slate-600 @ 70%   | **Page chrome only** (`html` / `body` / `.fi-body` / `.fi-main-ctn`): 6px webkit gutter + 1px rail on `.fi-body::after`. Track uses `--tido-scrollbar-track` (`white` / `slate-800`). **All nested panels** (dropdowns, filters, column manager, slide-overs, centered modals, tables, changelog, notifications, global search `.results-container`, date-picker month list, `.custom-scrollbar`) use `scrollbar-color` only in Chromium — no `scrollbar-width: thin`, no `::-webkit-scrollbar` — so the OS thumb stays overlay-smooth. Firefox nested bars are thin inside `@supports not selector(::-webkit-scrollbar)`. Collapsed sidebar hides the nav gutter. |
...
5. **Scrollable panels** — Add `custom-scrollbar` on custom `overflow-y-auto` regions (icon picker grid, widget lists, calendar popover). **Page chrome only** keeps the 6px webkit gutter (`html` / `body` / `.fi-body` / `.fi-main-ctn`). All nested Filament scrollers — dropdowns, filters, column manager, slide-overs, centered modals, tables, changelog, notifications, global search `.results-container`, date-picker month list — are listed in the overlay group in `app.css` (`scrollbar-color` + `will-change: scroll-position` only; no Chromium `::-webkit-scrollbar`). Do not put `custom-scrollbar` on an inner non-scrolling list. Do **not** add `scrollbar-width: thin` or `::-webkit-scrollbar` on nested overflow, or wheel scrolling becomes a classic gutter.
