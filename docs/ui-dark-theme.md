...
| Custom scrollbar thumb (dark) | slate-700 @ 50% → hover slate-600 @ 70% | `.custom-scrollbar`, `.fi-dropdown-panel`, changelog + database notification `.fi-modal-window`. Page scroller + `.fi-sidebar-nav` use the same webkit overlay in Chromium (do **not** set `scrollbar-width` / `scrollbar-color` on those — it restores the native OS bar) |
