...
| Registration                    | `HouseholdRegistrationService` + `/admin/register` (email OTP)                          | Public Register → **new** household + Primary    |
...
3. **Register** — **Verified** (MH-007): `HouseholdRegistrationService`; public Sign Up at `/admin/register` (email OTP or Google verified pending).
...
- Filament stock `Register::handleRegistration()` / `User::create()` without `HouseholdRegistrationService`
