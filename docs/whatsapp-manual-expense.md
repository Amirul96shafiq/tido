...
| `source` | `whatsapp` |
| `image_path` | `null` (Recent Receipts shows **Manual expense**, no file link) |
...
WhatsApp text (manual format)
  ...
  → pending Expense + ExpenseItems (labels null)
  → debounced Manual expense received ack
  → ParseManualWhatsAppExpenseJob (Ollama text JSON → Finance Labels)
...
A total of *N* manual expense(s) saved and queued for AI parsing.
...
| Piece | Location |
|-------|----------|
| Parser | `App\Support\ManualWhatsAppExpenseParser` |
| Webhook | `App\Http\Controllers\Api\WhatsAppWebhookController` |
| Create job | `App\Jobs\ProcessManualWhatsAppExpenseJob` |
