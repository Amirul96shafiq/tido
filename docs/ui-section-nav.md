...
| `<x-hash-scroll />`                                      | Smooth scroll for SPA / global-search navigation to `#hash`; expense line-item search emits one result per match (`Item` + `#expense-item-{id}`), activates Line Items, expands the row, scrolls to its label; `:target` highlights the repeater row border in primary                 |
