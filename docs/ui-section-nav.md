...
3. On tab click: `preventDefault` → `scrollElementIntoView` → `history.replaceState` for the hash (avoids the browser’s instant jump). When mobile nav makes `.fi-main-ctn` the page scroller, assign `scrollTop` there — nested `scrollIntoView` / `scrollTo({ behavior: 'smooth' })` do not move that container. Desktop window scrolling still uses `scrollIntoView({ behavior: 'smooth' })`.
...
6. Resets tab strip horizontal scroll to the leftmost position when the page returns to the top (`resetTabsScrollAtPageTop`); the first section also forces `scrollLeft = 0` in `scrollActiveTabIntoView`. Other tabs shift `.fi-tabs` with `scrollLeft` only — do not call `scrollIntoView` on a tab (it scrolls ancestor page scrollers).
...
| Stable anchor `id`s                                      | Target anchors for tabs, hash scroll, global search — keep `Section::make(...)->id('kebab-case')` on the parent; defer only inner `Schema::make()->deferLoading()` content, not the section shell                                                                                                                                                                                                                                                                                                                                |
| `scrollToSection` / `onNavClick`                         | Click intercept + scroll (in the Blade Alpine `x-data`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `pageScroller` / `scrollElementIntoView`                 | Resolve `.fi-main-ctn` when it is `overflow-y: auto\|scroll` (mobile nav) and set `scrollTop`; otherwise `scrollIntoView` on the window scroller                                                                                                                                                                                                                                                                                                                                                                                 |
| `<x-hash-scroll />`                                      | Smooth scroll for SPA / global-search navigation to `#hash`; expense line-item search emits one result per match (`Item` + `#expense-item-{id}`), activates Line Items, expands the row, scrolls to its label, and highlights the repeater row; other section hashes add `.tido-section-search-highlight` and use `:target` on the bordered `.fi-section` (schema ids live on `.fi-sc-section`). Tab clicks set `tido-suppress-search-highlight` and call `tidoClearSearchHighlights` so in-page tabs do not keep a search ring. |
...
| ----------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| Sticky tab click                                | Alpine `scrollToSection` → `scrollElementIntoView` (`preventDefault` + `replaceState`)                  |
| Global search / external `#hash` / SPA navigate | `<x-hash-scroll />` (`hashchange` / `livewire:navigated`) — same nested `.fi-main-ctn` `scrollTop` path |
...
5. Click intercept via `onNavClick` + `scrollElementIntoView` + hash `replaceState`.
...
- Call `scrollIntoView` on a tab link to bring it into the strip — that scrolls ancestor page scrollers. Adjust `.fi-tabs` `scrollLeft` instead.
