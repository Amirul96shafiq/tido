...
On Edit Profile Personalize, stylized background uses a custom field status pill can sit next to the control; dehydrate via `Hidden::make('stylized_background_enabled')`. Both previews share [`panel-preview-chrome`](../resources/views/components/tido/panel-preview-chrome.blade.php) (real sidebar logos; topbar profile circle only; live panel art from `images/bg-l.png` / `bg-d.png`). Each preview cross-follows the other control: sidebar collapse state from `$store.sidebar`, stylized background from entangled `data.stylized_background_enabled`.
