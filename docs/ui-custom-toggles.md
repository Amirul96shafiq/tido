...
Prefer Filament’s native `Toggle::make()` whenever the value lives in form/`$wire` state. Use a **custom Blade toggle** only when the control must bind to Alpine / `localStorage` (or another client store) that Livewire cannot reliably own — e.g. Filament sidebar `$store.sidebar` on Edit Profile.

**Canonical example:** [`resources/views/filament/schemas/components/sidebar-mode-field.blade.php`](../resources/views/filament/schemas/components/sidebar-mode-field.blade.php)
...
Match Filament’s field wrapper so the toggle lines up with native fields (e.g. Stylized Background):
...
6. Prefer copy impersonal — see [ui-copy-style.md](ui-copy-style.md).

## Related

- Profile Personalize: [`EditProfile.php`](../app/Filament/Pages/Auth/EditProfile.php), [`sidebar-mode-field.blade.php`](../resources/views/filament/schemas/components/sidebar-mode-field.blade.php)
