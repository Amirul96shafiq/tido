...
**Public Sign Up** (`/admin/register`): email, password, and a 6-digit email confirmation code—or **Continue with Google** with a verified Gmail pending state—create a **new household** and Primary via `HouseholdRegistrationService`. An existing Primary Gmail signs in and links Google instead of creating a second household.
