...
description: >-
  ...
  and Google Drive sync. Use when working on Invoice/InvoiceItem/Label/Budget/
...
| Model | Role |
|-------|------|
| `Invoice` | Receipt header: merchant, amounts, status, image/PDF document, `raw_ai_response`, `receipt_hash`; `payment_method_id`; optional `family_member_id` (**Uploaded By**); WhatsApp message and file metadata |
...
| `FamilyMember` | Household contact: WhatsApp allowlist + optional panel login; invoices attributed via `family_member_id` |
...
Attribution: `family_member_id` null = Primary; set from WhatsApp sender (`InvoiceSenderAttribution`) or acting family-member user. Mutate ACL: `HouseholdAccess::canMutateInvoice()` / `InvoicePolicy`.
...
| Household ACL | `App\Support\HouseholdAccess`, `App\Policies\InvoicePolicy` |
| Attribution / spender | `App\Support\InvoiceSenderAttribution`, `App\Support\DashboardSpenderScope` |
...
7. Invoice mutate paths: respect `HouseholdAccess::canMutateInvoice()`
